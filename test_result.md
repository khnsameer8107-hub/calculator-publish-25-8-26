...
agent_communication:
    ...
    - agent: "testing"
      message: "VERIFICATION AFTER frontend/.env RECREATION (2026-08-23): All verification tests PASSED. (a) App loads successfully through preview URL - NO 'Packager is not running' error, NO connection error, NO blank/white screen. (b) Home screen renders correctly with CalcHub header, search bar, 'Fast, private & offline' banner, Calculator/Scientific/History cards, Finance section (GST Calculator, Currency Converter, SIP & Investment, Loan/EMI), and Converters section. (c) Bottom tab navigation works perfectly - all 4 tabs (Home, Calculator, Finance, Convert) switch correctly without crashes. (d) Quick smoke tests PASSED - GST Calculator opens with input fields (Base Amount, GST Rate with quick select buttons 5%/12%/18%/28%, Final Amount display), Currency Converter opens with amount input, FROM/TO dropdowns (USD/INR), swap button, conversion result (1 USD = ₹83.2), and offline rates disclaimer. NO red error overlays detected anywhere. Console logs show only minor React Native Web warnings (deprecated props) which are cosmetic. App is fully functional after .env recreation."
...
## BUGFIX (frontend/.env recreation):
##     -agent: "main"
##     -message: "ROOT CAUSE: /app/frontend/.env was missing. /entrypoint.sh patches packager vars ONLY when this file exists (guarded by `if [ -f /app/frontend/.env ]`), so with it missing, EXPO_PACKAGER_HOSTNAME/PROXY_URL/PUBLIC_BACKEND_URL/TUNNEL_SUBDOMAIN were never provided to Metro. Web preview still worked (ingress routes to :3000) but Expo Go / QR on a real phone embedded the wrong host -> 'Packager is not running' on device. FIX: recreated /app/frontend/.env with the 4 packager vars set to preview_endpoint (https://mobile-exec-6.preview.emergentagent.com) + EXPO_TUNNEL_SUBDOMAIN + EXPO_USE_FAST_RESOLVER=1 + METRO_CACHE_ROOT. Restarted expo: log now shows 'env: load .env' + all vars exported, 'Fast resolver is enabled', :3000 HTTP 200. VERIFIED by testing agent: app loads, Home renders, tab nav works, GST + Currency screens render, no packager error, no red overlays."

## DEPLOYMENT FIX (EAS Android build - missing lockfile) - 2026-08-24
##     -agent: "main"
##     -message: "EAS Android build failed with 'No lockfile found in the project directory'. ROOT CAUSE: package.json packageManager field is yarn@1.22.22 (EAS uses yarn, requires yarn.lock) but yarn.lock was missing — only package-lock.json (npm) existed, so the deployment source archive shipped without yarn.lock. FIX: (1) regenerated /app/frontend/yarn.lock via `yarn install` (349KB, not gitignored so it ships in deploy archive); (2) removed conflicting /app/frontend/package-lock.json for a single consistent yarn lockfile. Also created /app/backend/.env (MONGO_URL/DB_NAME/CORS_ORIGINS) and added --tunnel flag + @expo/ngrok for Expo packager. deployment_agent re-check: PASS, no blockers. Need regression smoke test that the running app still loads after lockfile change."
##
## VERIFICATION: App Display Name Change (CalcHub → Calculator) - 2026-08-23
##     -agent: "testing"
##     -message: "COMPREHENSIVE VERIFICATION COMPLETED after app.json name change from 'CalcHub' to 'Calculator'. ALL TESTS PASSED: (a) App loads successfully through preview URL - NO 'Packager is not running' error, NO connection error, NO blank/white screen, NO red error overlay. (b) Home screen renders perfectly - CalcHub header visible (in-app text expected to remain), tagline 'Every calculator, one place', search bar, 'Fast, private & offline' banner, quick access cards (Calculator, Scientific, History), Finance section (GST Calculator, Currency Converter, SIP & Investment, Loan/EMI), Converters section (Age Calculator, Area Converter, Temperature, etc.). (c) Bottom tab navigation works flawlessly - all 4 tabs (Home, Calculator, Finance, Convert) switch correctly without crashes. (d) Core calculation smoke tests PASSED - Calculator: 12+8×2=28 (correct with operator precedence), Temperature converter: 0°C = 32°F (correct). (e) Offline functionality verified - NO external network requests detected (only 6 local requests), app is truly offline. (f) NO console errors, NO red error overlays detected. CONCLUSION: The app.json display name change did NOT break anything. App is fully functional and stable."

## REGRESSION SMOKE TEST: yarn.lock Regeneration - 2026-08-24
##     -agent: "testing"
##     -message: "REGRESSION SMOKE TEST PASSED after yarn.lock regeneration and package-lock.json removal (EAS deployment fix). ALL CRITICAL CHECKS PASSED: (a) App loads successfully through preview URL (https://5272c729-16df-43ab-81c6-af3af97f5f4c.preview.emergentagent.com) - NO 'Packager is not running' error, NO connection error, NO blank/white screen, NO red error overlay. (b) Home screen renders correctly with all expected elements: CalcHub header, tagline 'Every calculator, one place', search bar, 'Fast, private & offline' banner with '18 tools · works with no internet', quick access cards (Calculator, Scientific, History), Finance section (GST Calculator, Currency Converter, SIP & Investment, Loan/EMI), Converters section (Age Calculator, Area Converter, Length, Mass, Volume, Speed, Temperature, BMI, Date, Discount). (c) Bottom tab navigation works perfectly - all 4 tabs (Home, Calculator, Finance, Convert) switch correctly without crashes. Calculator tab shows full calculator interface with all buttons (AC, 0-9, operators, =). Finance tab displays all finance tools. Convert tab shows all converters. (d) Temperature converter verified: displays correct default conversion '0°C = 32°F' with input field, FROM dropdown (Celsius), TO dropdown (Fahrenheit), and swap button. (e) NO critical console errors detected - only 2 known React Native Web cosmetic warnings (deprecated props). (f) App is truly offline - only 4 local network requests detected, NO external API calls. (g) Metro bundler running correctly with Fast resolver enabled, tunnel connected. CONCLUSION: The yarn.lock regeneration and package-lock.json removal did NOT break the running app. All core functionality intact and working correctly."
